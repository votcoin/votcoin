...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars VOTCOIND and VOTCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

VOTCOIND=${VOTCOIND:-${1}/votcoind}
CLI=${VOTCOINCLI:-${1}/votcoin-cli}
...
$VOTCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$VOTCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
